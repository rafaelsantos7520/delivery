...
export function ProductCard({ product, onProductClick }: ProductCardProps) {
  ...
  const getCategoryIcon = (categoryName: string | null) => {
    if (!categoryName) return <Coffee className="h-4 w-4" />;
  ...
  return (
    <Card className="group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-0 shadow-lg overflow-hidden bg-gradient-to-br from-white to-gray-50 min-h-[400px] h-auto flex flex-col">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-3">
              <Badge className={`${getCategoryColor(product.categoryRelation?.name || null)} font-medium flex items-center gap-1`}>
                {getCategoryIcon(product.categoryRelation?.name || null)}
                {product.categoryRelation?.name || 'Sem categoria'}
              </Badge>
            </div>
            <CardTitle className="text-lg font-bold text-gray-800 group-hover:text-purple-600 transition-colors line-clamp-2">
              {product.name}
            </CardTitle>
          </div>
        </div>
